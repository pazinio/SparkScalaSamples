...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    // Sort by the max # of connections
    ...
    for (i <- 1 to 10) {
      val index = sorted.length - i
      val mostPopularName = namesRdd.lookup(sorted(index)._2)(0)
      println(s"$mostPopularName is the most popular superhero with ${sorted(index)._1} co-appearances.") 
    }    
  }
